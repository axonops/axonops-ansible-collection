...
- **cassandra role**: fixed `ansible.builtin.rpm_key` crashing with `'utf-8' codec can't decode byte 0xe9 ... invalid continuation byte` when importing the Apache Cassandra GPG keys on RedHat-family hosts. Apache's `KEYS` file has a non-UTF-8 byte in one contributor's comment, and `rpm_key`'s URL-fetch path strict-decodes the whole file before parsing. The role now downloads the file locally first and imports from disk, which avoids that code path; the whole import block is skipped on reconverge via a marker file so it stays idempotent. ([#124](https://github.com/axonops/axonops-ansible-collection/issues/124))

...
- **cassandra role**: fixed `ansible.builtin.rpm_key` crashing with `'utf-8' codec can't decode byte 0xe9 ... invalid continuation byte` when importing the Apache Cassandra GPG keys on RedHat-family hosts (ansible-core >= 2.18). Apache's `KEYS` file has one contributor's comment in Latin-1 instead of UTF-8, and `rpm_key` strict-decodes the whole file as text regardless of whether it's given a URL or a local path. The role now imports via `rpm --import` directly, which handles the raw bytes without decoding; the whole import block is skipped on reconverge via a marker file so it stays idempotent. ([#124](https://github.com/axonops/axonops-ansible-collection/issues/124))

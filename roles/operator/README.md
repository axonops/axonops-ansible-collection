...
The cert-manager install enables `crds.enabled: true` and waits for the `cert-manager-webhook` deployment to become ready before continuing. This ensures webhook certificates are available when the operator starts.
...
Any key you provide in `axonops_operator_helm_values` takes precedence over the base. Common overrides:
...
Each entry in `axonops_operator_platforms` MUST include `name` and `namespace`. The CR deployment mode is determined by which additional key is present — see [AxonOpsPlatform CR](#axonopsplatform-cr) below.

| Entry field | Required | Description |
|-------------|----------|-------------|
| `name` | Yes | Name of the `AxonOpsPlatform` resource and the target namespace |

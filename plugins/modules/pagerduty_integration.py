...
DOCUMENTATION = r'''
---
module: axonops_pagerduty_integration

short_description: Configure a PagerDuty integration for AxonOps

version_added: '1.0.0'

description: Configure a PagerDuty integration for AxonOps
...
def run_module():
    ...
    module = AnsibleModule(
        ...
        required_if=[('present', True, ('integration_key',))]
    )
    result = {
        'changed': False,
    }
    ...
    found_definition, error = axonops.find_integration_by_name_and_type(module.params['cluster'], 'pagerduty', module.params['name'])
    ...
    if existing_id:
        old_data = {
            ...
            'integration_key': found_definition['Params']['integration_key'],
            'present': True,
        }
    else:
    ...
    payload = {
        ...
        'params': {
            'name': new_data['name'],
            'integration_key': new_data['integration_key'],
        }
    }
    if existing_id:
